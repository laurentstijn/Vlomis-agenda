...
async function run() {
    ...
    const luc = users.find(u =>
        (u.vlomis_username && u.vlomis_username.toLowerCase().includes('luc')) ||
        (u.display_name && u.display_name.toLowerCase().includes('luc')) ||
        (u.vlomis_username && u.vlomis_username.toLowerCase().includes('ostaeyen'))
    );

    if (!luc) {
        console.log("Could not find any user matching 'luc'. Users available:");
